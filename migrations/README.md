...
- `0006_payments_mvp.sql` — таблица `payments` и credit status для jobs
- `0009_staging_credit_ledger_conflict_compat.sql` — staging compat для legacy/new `credit_ledger` полей
- `0010_credit_ledger_idempotency_backfill.sql` — backfill и жёсткий invariant для `credit_ledger.idempotency_key`
- `0011_credit_accounts_ledger_schema_align.sql` — `user_credit_accounts`, canonical ledger columns и compat trigger
- `0012_credit_ledger_trial_grant_compat.sql` — compat для `starter grant` / legacy `operation_type` constraint
- `0013_payments_provider_neutral_fields.sql` — provider-neutral поля платежей для Robokassa + будущих Telegram Stars
...
- Перед релизом payment/ledger кода на окружение, где этих объектов ещё нет, сначала применить `0011_credit_accounts_ledger_schema_align.sql`, затем выкатывать код.
- Перед внедрением Telegram Stars применить `0013_payments_provider_neutral_fields.sql`: она добавляет `currency`, `amount_provider_units`, provider payload/charge id и `delivery_channel`, сохраняя совместимость текущих Robokassa inserts через trigger.
- `0011` не применяется автоматически из Codex; rollout остаётся ручным через Supabase SQL Editor после явного подтверждения.
